...
export default function StatusIcon({ status, showQuestion }) {
    return (
        <>
            {status === "correct" && <img src={correctIcon} alt="Ícone de certo" data-test="zap-btn"/>}
            ...
            {status === "not answered" && <img onClick={showQuestion} src={play} alt="Ícone de play" data-test="play-icon"/>}
        </>
    )
}