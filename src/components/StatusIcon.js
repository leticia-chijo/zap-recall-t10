...
export default function StatusIcon({ status, showQuestion }) {
    return (
        <>
            {status === "correct" && <img src={correctIcon} alt="Ícone de certo" data-test="play-btn"/>}
            ...
            {status === "not answered" && <img onClick={showQuestion} src={play} alt="Ícone de play" data-test="zap-icon"/>}
        </>
    )
}